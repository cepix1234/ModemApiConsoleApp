...
if (
    /msie/.test(userAgent) &&
    parseInt(navigator.userAgent.toLowerCase().match(/msie ([\d.]+)/)[1]) < 9
) {
    var xmlHttp = null;
    var resData = null;
    ...
    if (
        ...
        resData.indexOf("</CurrentLanguage>") >= 0
    ) {
        resData = resData.replace(/\n/g, "");
        var curlang = resData
            .split("<CurrentLanguage>")[1]
            .split("</CurrentLanguage>")[0]
            .replace("-", "_");
        ...
        if (
            typeof resData === "string" &&
            resData.indexOf("'nosupport_IE8'") >= 0
        ) {
            langRes = resData
                .split("'nosupport_IE8'")[1]
                .split("\n")[0]
                .replace(": '", "")
                .replace("',", "");
        }
...
$.fn.val = function (data) {
    if (data || data === "") {
        ...
        if (
            this.hasClass("select_on_normal") ||
            this.hasClass("select_on_spanner") ||
            this.hasClass("statistic_select_statistics")
        ) {
...
function handleWlanEnabled(page) {
    ...
    function wanRelatedPageDisplay(page) {
        ...
        if (page === "wifinetworkssetting") {
            if (
                $("#wifinetworkssetting_page .pin_unuseable").css("display") ===
                "block"
            ) {
                return;
            }
            if (
                $("#wifinetworkssetting_unenabled_display").css("display") ===
                "block"
            ) {
                return;
            }
            ...
            if (
                $("#wifinetworkswps_page .pin_unuseable").css("display") ===
                "block"
            ) {
                return;
            }
            if (
                $("#wifinetworkswps_page_unuseable_tip").css("display") ===
                "block"
            ) {
                return;
            }
    ...
    if (gLinkTurboSwitch && gLinkTurboSwitch.linkTurboMode === "2") {
        if (
            page === "wifinetworkssetting" &&
            gmonitorstatus.WifiConnectionStatus !== "901"
        ) {
            ...
            (page === "wifinetworkssetting" || page === "wifinetworkswps") &&
            gmonitorstatus.WifiConnectionStatus === "901"
        ) {
            $("#" + wanDisconnectionPromptId).hide();
            $("#" + wanpromptId).show();
...
function generateLanguageSelect() {
    ...
    if (langLength > 1) {
        ...
        $("#language_select_list").html(
            '<div style="margin-bottom:-1px;" class="lang_top_normal hide"></div><div id="language_select_list_content_bg" class="hide"></div><div id="language_select_list_content"></div><div style="margin-top:-1px;" class="lang_bottom_normal hide"></div>'
        );
        ...
        for (var loop = 0; loop < langLength; loop++) {
            ...
            if (option === curlang) {
                pushHtml =
                    '<div id="language_select_list_{{loop}}" option="{{language}}" style="margin-top:-1px;" class="lang_middle_selected hide" onclick="changeLanguage(this)">';
            } else {
                pushHtml =
                    '<div id="language_select_list_{{loop}}" option="{{language}}" style="margin-top:-1px;" class="lang_middle_normal hide" onclick="changeLanguage(this)">';
            }
            pushHtml +=
                '<span lang-id="{{option}}" class="public_lang_left" option="{{language}}" ></span></div>';
            $("#language_select_list_content")
                .find(".scroll_text")
                .secureAppend(pushHtml, langTemplateData);
...
function utilStartConfirmDialog(
    notice,
    confirmcallback,
    canclecallback,
    confirmstr,
    ...
    $("#confirm_light").empty();
    var canclestring =
        typeof canclestr === "undefined" ? publicLang["btn.cancel"] : canclestr;
    var confirmsting =
        typeof confirmstr === "undefined"
            ? publicLang["btn.cofirm"]
            : confirmstr;
    var isShowStatus = typeof isshow === "undefined" ? false : isshow;
    var confirmhtml =
        '<div class="common_win_top"></div>' +
...
function utilStartSubmitDialog(noticeEX, btnId) {
    ...
    var submithtml;
    submithtml =
        '<div class="common_win_top"></div><div align="center" class="margin_bottom_box2 color_Darkgray color_background_white" style="font-size:16px;width:508px;padding-top:40px;min-height:136px;">' +
    ...
    if (btnId) {
        submithtml +=
            '<button id="{{btnId}}" style="margin-top:60px;margin-bottom: 8px" class="btn_normal_short" lang-id="btn.cancel">' +
            publicLang["btn.cancel"] +
            "</button>";
    }
...
function utilStartCommonDialog(noticeEX) {
    ...
    var submithtml =
        '<div class="common_win_top"></div><div align="center" class="margin_bottom_box2 color_Darkgray color_background_white" style="font-size:16px;width:508px;padding-top:40px;min-height:100px;">' +
...
function guideHref(addSearch) {
    ...
    var isSupportCaptive =
        GLOBAL.modules.captivePortalGuide === "1" ? true : false;
    var resSearch = "";
    ...
    if (
        ...
        if (isSupportCaptive && !isGuideSmall && !hasNoPhone) {
            device_differentiation = "0";
            return resSearch
                ? "../html/guidesmall.html" + resSearch
                : "../html/guidesmall.html";
        }
    } else {
        ...
        $xml.children().each(function () {
            var _childObj =
                $(this).children().size() > 0
                    ? _recursiveXml2Object($(this), objName)
                    : $(this).text();
            if (objName && objName === "sms") {
                if (
                    $(this).siblings().size() > 0 &&
                    $(this).siblings().get(0).localName === this.localName
                ) {
...
function SelectItem(obj) {
    ...
    if (
        $("#" + selectitemsid + " ." + shortselect.select_medium).length !== 0
    ) {
        classes = shortselect;
    }
    $("#" + selectitemsid + " ." + classes.select_medium).removeClass(
        classes.select_medium_selected
    );
    var selectitemsLength = $(
        "#" + selectitemsid + " ." + classes.select_medium
    ).length;
    if ($("#" + selectitemsid + " ." + classes.select_top).length === 0) {
        var showitemcount =
            selectitemsLength < SHOW_MAX_ITEM_COUNT
                ? selectitemsLength
                : SHOW_MAX_ITEM_COUNT;
        ...
        $("#" + selectitemsid + "_items :last-child").css(
            "height",
            classes.height - 1 + "px"
        );
    }
    ...
    if ((classes = normalselect)) {
        for (var i = 0; i < selectitemsLength; i++) {
            container = $(
                $("#" + selectitemsid + " ." + classes.select_medium)[i]
            ).find("div");
            content = $(
                $("#" + selectitemsid + " ." + classes.select_medium)[i]
            ).find("span");
            if (content.length === 1) {
                ...
                $($("#" + selectitemsid + " ." + classes.select_medium)[i]).css(
                    "line-height",
                    "40px"
                );
    ...
    $.ajax({
        ...
        error: function (XMLHttpRequest, textStatus) {
            try {
                if (jQuery.isFunction(errorCallback)) {
                    ...
                    window.location.hash === "#mobileconnection" &&
                    GLOBAL.isBridgeModeOpen
                ) {
        ...
        success: function (data) {
            ...
            if (typeof content === "string" || typeof content === "number") {
                if (
                    this.url.indexOf("/api/ussd/get") !== -1 &&
            ...
            if (ret.type === "error") {
                if (
                    ERROR_WRONG_SESSION_TOKEN === ret.error.code ||
                    ERROR_WRONG_SESSION === ret.error.code
                ) {
...
function ShowPassword(obj) {
    ...
    if (classlist.indexOf("ic_eye_open") >= 0) {
        ...
        if (
            ...
            typeof $(open_id + " " + "input").attr("placeholder") !==
                "undefined"
        ) {
        ...
        if (
            !isSupportPlaceholder() &&
            typeof $(close_id + " " + "input").attr("placeholder") !==
                "undefined"
        ) {
            var input_open_id = $(open_id + " " + "input").attr("id");
            var input_close_id = $(close_id + " " + "input").attr("id");
...
function preLoadEyeImg() {
    var eyeImg =
        '<div id="preLoadEyeImg"><div class="ic_guide_eye_open" style="opacity:0;height:0;margin:0"></div>\
...
function isEthernetConnect(page) {
    ...
    if (GLOBAL.modules.wanmanagement_enabled === "1") {
        getAjaxData(
            "api/dialup/multiWanInformation",
            function (ret) {
                if (ret.type === "response") {
                    if (
                        typeof ret.response.service_list !== "undefined" &&
                        ret.response.service_list !== ""
                    ) {
                        var wanData = $.makeArray(
                            ret.response.service_list.service
                        );
...
function resetTableHeight(itemClass, n) {
    ...
    for (var i = 0; i < itemEle.length; i++) {
        itemEle
            ...
            .eq(i)
            .parent()
            .height(mostHeight + "px");
        itemEle.children().eq(i).parent().addClass("table_content_line_item");
    }
    $("." + itemClass)
        .eq(n)
        .height(mostHeight + 10 + "px");
    $("." + itemClass)
        .eq(n)
        .addClass("table_content_line");
}
function clearAllErrorMsg() {
    $(".error_message").remove();
    if ($("#ipsec_first_authentication_select").val() === "sha1") {
        showErrorMsg(
            "ipsec_first_authentication",
            "IDS_vpn_ipsec_first_authentication_sha1_warn"
        );
    }
...
function showInputBorder(obj) {
    ...
    if (curElement && curElement.parent()) {
        if (curElement.parent().hasClass("input_normal")) {
            curElement
                .parent()
                .addClass("input_normal_selected")
                .removeClass("input_normal");
        }
        if (curElement.parent().hasClass("input_long")) {
            curElement
                .parent()
                .addClass("input_long_selected")
                .removeClass("input_long");
        }
    }
}
    ...
    if (curElement && curElement.parent()) {
        if (curElement.parent().hasClass("input_normal_selected")) {
            curElement
                ...
                .removeClass("input_normal_selected");
        }
        if (curElement.parent().hasClass("input_long_selected")) {
            curElement
                .parent()
                .addClass("input_long")
                .removeClass("input_long_selected");
        }
    }
}
...
function hideShortInputBorder(obj) {
    ...
    if (curElement && curElement.parent()) {
        curElement
            ...
            .removeClass("short_input_selected");
    }
}
    ...
    if (classVal && classVal.indexOf("input_normal") >= 0) {
        ...
        $("#" + divid)
            ...
            .each(function (index, element) {
                ...
                if (
                    classVal &&
                    classVal.indexOf("input_normal_selected") >= 0
                ) {
                    $(element)
                        .addClass("input_error")
                        .removeClass("input_normal_selected");
...
function escapeURL(source) {
    ...
    if (!checkUrl(source)) {
        ...
        if (
            copystr.indexOf("javascript") !== -1 ||
            copystr.indexOf("script") !== -1
        ) {
            return "/";
        }
...
$(function () {
    ...
    function cloneObject(inObj) {
        ...
        for (var i in inObj) {
            if (
                (inObj[i] && typeof inObj[i] === "object") ||
                typeof inObj[i] === Object
            ) {
    ...
    function compareObject(inObj1, inObj2) {
        ...
        for (i in inObj1) {
            if (
                (inObj1[i] && typeof inObj1[i] === "object") ||
                typeof inObj1[i] === Object
            ) {
	...
	<div id="scroll_text_' +
                    i +
                    '" class="scroll_text"></div>\
	</div>'
            );
            $selector.append(
                '<div id="scroll_right_' +
                    i +
                    '" class="scroll_right" style="display:none">\
	<div id="scroll_bar_' +
                    i +
                    '" class="scroll_bar" style="min-height:40px;"></div>\
	</div>'
            );
        },
        intervalResize: function ($selector, i) {
            var self = this;
            if (
                $("#scroll_text_" + i).length === 0 &&
                self.intervalResizeObj[i]
            ) {
        ...
        bindEvent: function (i) {
            ...
            $targetDom.on(
                ...
                function (e) {
                    if (
                        !self.touchData.startSelector ||
                        !self.touchData.startBoolear ||
                        !$("#" + self.touchData.startSelector).length
                    ) {
                        return;
                    }
            ...
            $(document).on("mousemove", function (e) {
                if (
                    !self.mouseData.startSelector ||
                    !self.mouseData.startBoolear ||
                    !$("#" + self.mouseData.startSelector).length
                ) {
                    return;
                }
                window.getSelection
                ...
                var barTop =
                    e.pageY - self.mouseData.startY + self.mouseData.curTop;
                self.resizeTop($("#" + id), barTop);
            });
        ...
        destroy: function (selectorS) {
            ...
            $.each($(selectorS), function (index, selector) {
                ...
                $(selector)
                    .find(".scroll_left")
                    .replaceWith($scrollText.contents());
    ...
    EMUI.ObjController = EMUI.Object.extend({
        ...
        load: function (callback, bAsync) {
            ...
            if (
                myurl === "/api/wlan/multi-basic-settings" ||
                myurl === "/api/wlan/basic-settings"
            ) {
                headers["Login-Type"] = "Scram";
            }
                ...
                success: function (data) {
                    ...
                    if (myurl === "/system/ioc_device_capacity.json") {
                        ...
                        if (self.contentType.indexOf("json") !== -1) {
                            if (
                                Object.prototype.toString.call(resData) ===
                                "[object String]"
                            ) {
                            ...
                            if (
                                ...
                                if (
                                    this.url.indexOf("/api/ussd/get") !== -1 &&
                                    resData.indexOf("content") !== -1
                                ) {
                                    resData =
                                        smsContentDeleteWrongChar(resData);
                                } else if (
                                    this.url.indexOf("/api/sdcard/sdcard") !==
                                    -1
                                ) {
        ...
        base64encode: function (str) {
            ...
            while (i < input.length) {
                ...
                outputStr +=
                    keyStr.charAt(encry1) +
                    keyStr.charAt(encry2) +
                    keyStr.charAt(encry3) +
                    keyStr.charAt(encry4);
            }
            return outputStr;
        },
        ...
        requestEncpubkey: function () {
            ...
            if (typeof classify !== "undefined" && classify !== "hilink") {
                if (
                    typeof storage.e === "undefined" ||
                    typeof storage.n === "undefined"
                ) {
                    $("body").hide();
            ...
            if ($xml.children().size() > 0) {
                ...
                $xml.children().each(function () {
                    ...
                    if (
                        ...
                        if (
                            typeof _obj[this.tagName] === "undefined" ||
                            _obj[this.tagName] === null
                        ) {
                            _obj[this.tagName] = [];
                        }
                        _obj[this.tagName].push(_childObj);
        ...
        clearAllErrorLabel: function () {
            if (
                AJAX_HEADER + this.objName !==
                    "/api/language/current-language" &&
                AJAX_HEADER + this.objName !== "/api/device/vendorname" &&
                AJAX_HEADER + this.objName !== "/api/vpn/ipsec_settings"
            ) {
                clearAllErrorMsg();
            }
        },
        ...
        checkVisibleChar: function (str) {
            ...
            for (var i = 0; i < str.length; i++) {
                ...
                if (
                    numCharStr > MACRO_SUPPORT_CHAR_MAX ||
                    numCharStr < MACRO_SUPPORT_CHAR_MIN
                ) {
                    return false;
                }
            }
        ...
        postData: function (data, callback, bAsync) {
            ...
            $.ajax({
                ...
                error: function (XMLHttpRequest, textStatus) {
                    if (
                        XMLHttpRequest.status === "12030" ||
                        XMLHttpRequest.status === "12031" ||
                        XMLHttpRequest.status === "12019" ||
                        XMLHttpRequest.status === "400"
                    ) {
                        self.postData();
                        return;
                    }
                ...
                success: function (res) {
                    ...
                    if (self.contentType.indexOf("json") !== -1) {
                        ...
                        if (typeof localData === "string") {
                            if (
                                myurl.indexOf("/api/sms/sms-list") !== -1 &&
                                localData.indexOf("messages") !== -1
                            ) {
                                localData =
                                    smsContentDeleteWrongChar(localData);
                            }
                            if (!window.ActiveXObject) {
                                var parser = new DOMParser();
                                xml = parser.parseFromString(
                    ...
                    if (
                        typeof xmlRet.error !== "undefined" &&
                        myurl.indexOf("/api/user/session") === -1
                    ) {
                        if (
                            xmlRet.error.code == ERROR_SYSTEM_NO_RIGHTS ||
                            xmlRet.error.code == ERROR_WRONG_TOKEN ||
                            xmlRet.error.code == ERROR_WRONG_SESSION_TOKEN ||
                            xmlRet.error.code == ERROR_WRONG_SESSION
                        ) {
                            if (
                                xmlRet.error.code == ERROR_WRONG_SESSION_TOKEN
                            ) {
                                if (
                                    ...
                                    if (
                                        EMUI.loginallowedObjController.content
                                            .response.hilink_login === "0"
                                    ) {
                                        EMUI.TokenObjController.load(
                                            function () {},
                                            false
                                        );
                                        setTimeout(function () {
                        ...
                        if (GLOBAL.voiceBusyNotice === "1") {
                            if (
                                xmlRet.error.code.toString() ===
                                ERROR_VOICE_BUSY.toString()
                            ) {
                        ...
                        myurl.indexOf("/api/user/challenge_login") !== -1 ||
                        myurl.indexOf("/api/user/authentication_login") !==
                            -1 ||
                        myurl.indexOf("/api/user/password_scram") !== -1
                    ) {
                        if ($.isArray(gRequestVerificationToken)) {
                            gRequestVerificationToken = [];
                        }
                ...
                complete: function (XMLHttpRequest, textStatus) {
                    ...
                    if (
                        xmlHeaders.indexOf("__RequestVerificationTokenone") >= 0
                    ) {
                        gRequestVerificationToken.push(
                        ...
                        xmlHeaders.indexOf("__RequestVerificationToken") >= 0
                    ) {
                        gRequestVerificationToken.push(
                            self.getTokenFromHeader(
                                xmlHeaders,
        ...
        getTokenFromHeader: function (headers, tokenHeader) {
            var tokenindex =
                headers.indexOf(tokenHeader) + tokenHeader.length + 1;
...
$(function () {
    ...
    EMUI.doubleFrequencyConObjCtrl = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            ...
            EMUI.doubleFrequencyConObjCtrl.initStatus =
                GLOBAL.modules.isdbFrequencyStatus;
        },
    });
    ...
    EMUI.intelligenceConStaObjCtrl = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            ...
            if (
                intellStatus[0].type === "error" ||
                (intellStatus && intellStatus[0].WifisyncStatus)
            ) {
    ...
    EMUI.globalDetectSmartUpgVersionController = EMUI.ObjController.extend({
        ...
        toUpgradePage: function () {
            ...
            EMUI.indexAutoUpgradeController.sendAutoUpg(
                EMUI.globalSmartUpgradeStatusController.fromjump
            );
    ...
    EMUI.globalSmartUpgradeStatusController = EMUI.ObjController.extend({
        ...
        generatePopWinInfo: function (dataItem) {
            if (dataItem !== "") {
                ...
                var popHtml =
                    '<div lang-id="upgrade.pop.info">' +
                    publicLang["upgrade.pop.info"] +
                    "</div>" +
                    '<table style="margin-top:10px;" cellpadding="0" cellspacing="0" frame=void rules=none>' +
                    '<tr style="height:30px;"><td lang-id="upgrade.oldversion">' +
                    publicLang["upgrade.oldversion"] +
                    '</td><td style="width:20px;">&nbsp;</td><td style="direction:ltr;">' +
                    xss(oldVerion) +
                    "</td></tr>" +
                    '<tr style="height:40px;"><td lang-id="upgrade.newversion">' +
                    publicLang["upgrade.newversion"] +
                    '</td><td style="width:20px;">&nbsp;</td><td style="direction:ltr;">' +
                    xss(newVersion) +
                    "</td></tr></table>" +
                    '<div class="onekey_red" style="margin-top:-5px;" lang-id="upgrade.onekey.body">' +
                    ...
                    '</div><div class="color_Darkgray" style="font-size: 14px;width: 430px;line-height:20px;" lang-id="index_autoUpg_label">' +
                    publicLang["index_autoUpg_label"] +
                    "</div></div>" +
                    '<div class="clearboth" style="margin-top:50px;text-align:center;"><button class="btn_normal_long" onclick="EMUI.globalDetectSmartUpgVersionController.toUpgradePage();" lang-id="onlineupg.toUpgrade">' +
                    publicLang["onlineupg.toUpgrade"] +
                    "</button></div>";
                EMUI.popWinController.addPopItem(
                    "upgrade",
                    "upgrade.newversion.notice",
                    popHtml
                );
            } else {
                EMUI.popWinController.closePopItem("upgrade");
            ...
            if (typeof result !== "undefined") {
                for (var i = 0; i < result.length; i++) {
                    if (result[i]["IsMainDevice"]) {
                        if (
                            result[i]["UpdateState"] === 18 ||
                            result[i]["UpdateState"] === 33 ||
                            result[i]["UpdateState"] === 255
                        ) {
        ...
        getsuccessProc: function (result) {
            ...
            if (typeof result !== "undefined") {
                for (var i = 0; i < result.length; i++) {
                    if (result[i]["IsMainDevice"]) {
                        if (
                            result[i]["UpdateState"] !== 23 &&
                            result[i]["UpdateState"] !== 24 &&
                            result[i]["UpdateState"] !== 25 &&
                            result[i]["UpdateState"] !== 26 &&
                            result[i]["UpdateState"] !== 27
                        ) {
    ...
    EMUI.getNewComponentInfoController = EMUI.ObjController.extend({
        ...
        toUpgradePage: function () {
            ...
            EMUI.indexAutoUpgradeController.sendAutoUpg(
                EMUI.getNewComponentInfoController.fromjump
            );
            EMUI.popWinController.closePopItem("upgrade");
            window.location.href = "#upgrade";
        },
        getsuccessProc: function (data) {
            if (data.type === "response") {
                ...
                if (typeof responseData.ComponentList !== "undefined") {
                    ...
                    var newVersion = "";
                    if (
                        typeof responseData.ComponentList.Component !==
                        "undefined"
                    ) {
                        var updateComponent =
                            responseData.ComponentList.Component;
                    ...
                    if (deviceInfo && deviceInfo["type"] === "response") {
                        ...
                        var oldVerion = "";
                        if (
                            typeof deviceData["SoftwareVersion"] !==
                                "undefined" &&
                        ...
                        if (
                            oldVerion === "" &&
                            typeof deviceData["WebUIVersion"] !== "undefined" &&
                            deviceData["WebUIVersion"] !== ""
                        ) {
                    ...
                    var popHtml =
                        ...
                        publicLang["index_autoUpg_label"] +
                        "</div></div>" +
                        '<div class="clearboth" style="margin-top:50px;text-align:center;"><button class="btn_normal_long" onclick="EMUI.getNewComponentInfoController.toUpgradePage();" lang-id="onlineupg.toUpgrade">' +
                        publicLang["onlineupg.toUpgrade"] +
                        "</button></div>";
                    EMUI.popWinController.addPopItem(
                        "upgrade",
    ...
    EMUI.headerStautsController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            ...
            if (
                typeof responceData["SmsStorageFull"] !== "undefined" &&
                responceData["SmsStorageFull"] === "1" &&
                apiPinStatus.SimState !== CONST.MACRO_NO_SIM_CARD
            ) {
                $("#sms_not_full").hide();
                $("#sms_full_new").hide();
                $("#sms_full").hide();
            ...
            if (
                GLOBAL.modules.bridgemode_onlineupdate_enable !== "1" &&
                GLOBAL.isBridgeModeOpen
            ) {
                EMUI.popWinController.closePopItem("upgrade");
                $("#header_new_version").hide();
            ...
            if (
                !(
                    GLOBAL.cradleStatus &&
                    GLOBAL.cradleStatus.cradlestatus === "1" &&
                    GLOBAL.cradleStatus.connectstatus === "901"
                ) &&
                !(
                    GLOBAL.modules.wifioffload_enabled === "1" &&
                    GLOBAL.monitoringStatus.WifiConnectionStatus === "901"
                ) &&
    ...
    EMUI.pubStatisticFeatureController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (res) {
            if (res.type === "response") {
                this.isDailyLimitEnable =
                    res.response.daily_statistic_limit_enable === "1"
                        ? true
                        : false;
            }
        },
    ...
    EMUI.pubMonthStatisticsController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (monthData) {
            if (monthData.type === "response") {
                ...
                gTraffic.usagetraffic =
                    parseInt(responseData["CurrentMonthDownload"], 10) +
                    parseInt(responseData["CurrentMonthUpload"], 10);
                if (
                    typeof responseData["roam_month_download"] !==
                        "undefined" &&
                    typeof responseData["roam_month_upload"] !== "undefined"
                ) {
                    gTraffic.roamUsagetraffic =
                        parseInt(responseData["roam_month_download"], 10) +
                        parseInt(responseData["roam_month_upload"], 10);
                } else {
                    gTraffic.roamUsagetraffic = 0;
                }
                gTraffic.dailydataused = parseInt(
                    responseData["CurrentDayUsed"],
                    10
                );
            }
            ...
            if (
                gTraffic["usagetraffic"] >= gTraffic["dataLimit"] &&
                gTraffic["dataLimit"] !== 0 &&
                gTraffic["setmonthdata"] === "1"
            ) {
                popHtml =
                    '<div lang-id="statistic.flowOver">' +
                    publicLang["statistic.flowOver"] +
                    "</div>" +
                    '<div style="margin:0 auto;margin-top:50px;text-align:center;"><button class="btn_normal_long" lang-id="statistic.setagain" onclick="EMUI.pubMonthStatisticsController.toStatistic();">' +
                    publicLang["statistic.setagain"] +
                    "</button></div>" +
                    '<div class="clearboth" style="margin-top:10px;text-align:center;"><div class="home_font_style pointer" onclick="EMUI.pubMonthStatisticsController.ignorStatistic();" lang-id="statistic.ignor">' +
            ...
            if (
                gTraffic["daydatalimit"] !== 0 &&
                gTraffic["dailydataused"] >= gTraffic["daydatalimit"] &&
                gTraffic["SetDayData"] === "1"
            ) {
                popHtml =
                    '<div lang-id="statistic.flowOver">' +
                    publicLang["statistic.flowOver"] +
            ...
            if (
                ...
                gTraffic["setmonthdata"] === "1" &&
                gTraffic["roamUsagetraffic"] >= gTraffic["roamFlow"]
            ) {
                popHtml =
                    '<div lang-id="statistic.roamexceed">' +
                    publicLang["statistic.roamexceed"] +
                    "</div>" +
                    '<div style="margin:0 auto;margin-top:50px;text-align:center;"><button class="btn_normal_long" lang-id="statistic.setagain" onclick="EMUI.pubMonthStatisticsController.toStatistic();">' +
                    publicLang["statistic.setagain"] +
                    "</button></div>" +
                    '<div class="clearboth" style="margin-top:10px;text-align:center;"><div class="home_font_style pointer" onclick="EMUI.pubMonthStatisticsController.ignorRoamStatistic();" lang-id="statistic.ignor">' +
                    publicLang["statistic.ignor"] +
                    "</div></div>";
                if (GLOBAL.isSecondaryAccount) {
                    popHtml =
            ...
            if (
                (isChangeDaily && this.currentDaily) ||
                (isChangeMonth && this.currentMonth)
            ) {
                var isNeedShowAgain = false;
                var contentLength = EMUI.popWinController.content.length;
                for (var i = 0; i < contentLength; i++) {
                    if (
                        EMUI.popWinController.content[i]["moduleName"] ===
                        "statistic"
                    ) {
        ...
        getsuccessProc: function (startData) {
            if (startData.type === "response") {
                ...
                if (datalimit.length === 0) {
                    ...
                    datalimit = parseInt(
                        datalimit.substring(0, datalimit.length - 2),
                        10
                    );
                ...
                if (
                    typeof responseData["RoamSetMonthData"] !== "undefined" &&
                    responseData["RoamSetMonthData"] === "1"
                ) {
                    gTraffic.supportRoamOver = true;
                    var roamDataLimit = 0;
                    if (typeof responseData["RoamDataLimit"] !== "undefined") {
                        ...
                        if (roamDataLimit.length === 0) {
                            ...
                            unit = roamDataLimit.substring(
                                roamDataLimit.length - 2
                            );
                            roamDataLimit = parseInt(
                                roamDataLimit.substring(
                                    0,
                                    roamDataLimit.length - 2
                                ),
                                10
    ...
    EMUI.pubDailyStatisticsStartDateController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (dailyData) {
            ...
            if (dailyData.type === "response") {
                ...
                gTraffic.dailyturnoffdataenable =
                    responseData["dailyturnoffdataenable"];
                gTraffic.dailyturnoffdataswitch =
                    responseData["dailyturnoffdataswitch"];
                gTraffic.dailyturnoffdataflag =
                    responseData["dailyturnoffdataflag"];
                ...
                if (daydatalimit.length === 0) {
                    ...
                    daydatalimit = parseInt(
                        daydatalimit.substring(0, daydatalimit.length - 2),
                        10
                    );
    ...
    EMUI.globalStatusController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            ...
            var newNetMode = null;
            if (
                typeof monitoringStatus["CurrentNetworkTypeEx"] !==
                    "undefined" &&
                monitoringStatus["CurrentNetworkTypeEx"] !== ""
            ) {
            ...
            if (winHref.indexOf("/html/content.html") >= 0) {
                EMUI.getWirelessProfileName.load(function (profileData) {
                    if (profileData.type === "error") {
                        ...
                        if (
                            apiPinStatus.SimState === CONST.MACRO_NO_SIM_CARD ||
                            apiPinStatus.PinOptState === CONST.MACRO_CPIN_FAIL
                        ) {
                    ...
                    if (
                        GLOBAL.isSecondaryAccount ||
                        !GLOBAL.modules.sms_enabled ||
                        GLOBAL.modules.sms_enabled === "0"
                    ) {
                        $("#header_sms_info").hide();
                    } else {
    ...
    EMUI.getWirelessProfileName = EMUI.ObjController.extend({
        ...
        getPlmnName: function (wirelessData, monitoringStatus) {
            var plmnName = "";
            if (
                typeof wirelessData["ShortName"] !== "undefined" &&
                wirelessData["ShortName"].length > 0
            ) {
                plmnName = xss(wirelessData["ShortName"]);
            } else if (
                typeof wirelessData["FullName"] !== "undefined" &&
                wirelessData["FullName"].length > 0
            ) {
                plmnName = xss(wirelessData["FullName"]);
            } else if (
                typeof wirelessData["Numeric"] !== "undefined" &&
                wirelessData["Numeric"].length > 0
            ) {
            ...
            if (
                monitoringStatus["RoamingStatus"] === "1" &&
                monitoringStatus["ServiceStatus"] === "2"
            ) {
                if (plmnName === "") {
                    plmnName =
                        '<span lang-id="IDS_dialup_label_roaming">' +
                    ...
                    if (data.response && data.response.userlevel === "0") {
                        if (rediretcturl) {
                            ...
                            window.location.href =
                                GLOBAL.INDEX_NO_SIMREDIRECT_PAGE_URL;
                        }
                    }
                });
            }, t);
    ...
    var versionName = {};
    var tempLangId = [
        "device_privacy_statement_title_update",
        "privacy_notice_title_update",
        "device_user_agreement_title_update",
        "privacy_notice_title_china_update",
        "privacy_notice_title_oversea_update",
    ];
    EMUI.vendorNameController = EMUI.ObjController.extend({
        ...
        postSuccessProc: function (data) {
            if (data.response) {
                versionName[LANGUAGE_DATA.current_language] =
                    data.response.version_name;
            }
        },
        replaceLangId: function () {
            ...
            tempLangId.forEach(function (v) {
                publicLang[v] = publicLang[v].replace(
                    "%s",
                    versionName[LANGUAGE_DATA.current_language]
                );
            });
    ...
    function aiLife() {
        var newLangId = [
            "ailife_download_update",
            "index_ailife_update",
            "smart_life_app_update",
            "guidesmall_smarthome_update",
        ];
        if (GLOBAL.modules && GLOBAL.modules.vendor_enable !== "1") {
            ...
            newLangId.forEach(function (v) {
                ...
                publicLang[vendorV] = publicLang[v].replace(
                    "%s",
                    versionName[LANGUAGE_DATA.current_language]
                );
            });
    ...
    EMUI.privacyNoticeInfoController = EMUI.ObjController.extend({
        ...
        checkPrivacyNotice: function () {
            ...
            if (GLOBAL.modules && GLOBAL.modules.gdpr_enabled === "1") {
                if (LANGUAGE_DATA.current_language) {
                    self.objName =
                        "app/privacypolicy?lang=" +
                        LANGUAGE_DATA.current_language;
                }
                self.load(function (resData) {
                    if (
                        GLOBAL.isSecondaryAccount &&
                        (resData.OperatorEULAUpdate === 1 ||
                            resData.OperatorPrivacyPolicyUpdate === 1)
                    ) {
                        ...
                        !GLOBAL.isSecondaryAccount &&
                        (resData.IsPrivacyPolicyUpdate === 1 ||
                            resData.IsEULAUpdate === 1) &&
                        resData.change_status !== 2
                    ) {
                        self.isPolicyUpdate = true;
        ...
        popPrivacyNotice: function (callback) {
            ...
            if (curHref.indexOf("guidesmall") !== -1) {
                ...
                if (
                    GLOBAL.modules.vendor_enable &&
                    GLOBAL.modules.vendor_enable === "1"
                ) {
                    $("#content_title").text(
                        publicLang["wireless_broadband_device_privacy_notice"]
                    );
                    $("#content_title").attr(
                        "lang-id",
                        "wireless_broadband_device_privacy_notice"
                    );
                } else {
                    $("#content_title").text(
                        publicLang["privacy_notice_title_oversea_update"]
                    );
                    $("#content_title").attr(
                        "lang-id",
                        "privacy_notice_title_oversea_update"
                ...
                EMUI.LanguageController.registerLanguageEx(
                    "agreenButton_description",
                    "agreen_tip_2",
                    "%l1",
                    "device_user_agreement_title_update",
                    EMUI.privacyNoticeInfoController.eulaUrl,
                    "#007DFF"
                );
                EMUI.LanguageController.registerLanguageEx(
                    "agreenButton_description",
                    ...
                    "%l2",
                    "device_privacy_statement_title_update",
                    EMUI.privacyNoticeInfoController.privacyPolicyUrl,
                    "#007DFF"
                );
                EMUI.LanguageController.transLangEx();
                $("#content_privacy").secureHtml(
                    EMUI.privacyNoticeInfoController.simplePrivacyPolicy
                );
                $("#agreenButton_description").secureHtml(
                    $("#agreenButton_description")
                        .html()
                        .replace("%s", publicLang["index.agreeBtn"])
                );
            } else {
                if (
                    curHref.indexOf("index.html") === -1 &&
                    curHref.indexOf("guide.html") === -1 &&
                    $("body #guide_privacy_page").length === 0
                ) {
                    var privacyHtml =
                    ...
                    if (!isPc) {
                        $("#guide_privacy_page")
                            .removeClass("out_win_content")
                            .addClass("phone_out_win_content");
                    } else {
                        $("#guide_privacy_page")
                            .removeClass("phone_out_win_content")
                            .addClass("out_win_content");
                    }
                }
                var privacyContent =
                    '<div style="padding-left:20px;padding-right:20px;"><div id="privacyTip" style="padding-bottom:5px"></div><div id="eulaTip"></div></div>';
                var privacyLink = "";
                if (GLOBAL.modules && GLOBAL.modules.chrlog_enable === "1") {
                    privacyLink +=
                        '<div id="guide_userExperience_content" style="padding-left:20px;padding-right:20px;padding-bottom:16px;">\
	...
	<div class="margin-left-10" id="user_experience_label"></div>\
                ...
                privacyLink +=
                    '<div id="agreen_tips" lang-id="" style="padding-left:20px;padding-right:20px;"></div>';
                var privacyBtn =
                    '<div align="center" style="margin:0 auto;margin-top:50px;">\
	...
	<button id="privacyAgreen" lang-id="" class="btn_normal_short margin-left-15"></button>\
                ...
                if ($("#guide_privacy_content #privacyTip").length === 0) {
                    $("#guide_privacy_content .scroll_text").secureHtml(
                        privacyContent
                    );
                }
                ...
                if (
                    curHref.indexOf("guidesmall.html") === -1 &&
                    curHref.indexOf("guide.html") === -1 &&
                    curHref.indexOf("index.html") === -1
                ) {
                    $("#submit_fade").show();
        ...
        privacyFirstPage: function () {
            ...
            $(document).on(
                "click",
                "#privacyDisagreen",
                EMUI.privacyNoticeController.disagreePrivacyNotice
            );
            if (!EMUI.privacyNoticeInfoController.isFirstUpdate) {
                ...
                $("#guide_privacy_title").text(
                    publicLang[
                        EMUI.privacyNoticeInfoController.titleObj[updateType]
                    ]
                );
                $("#guide_privacy_title").attr(
                    "lang-id",
                    EMUI.privacyNoticeInfoController.titleObj[updateType]
                );
                $("#privacyTip").secureHtml(
                    publicLang[
                        EMUI.privacyNoticeInfoController.contentObj[updateType]
                    ]
                );
                if (updateType === "0") {
                    EMUI.LanguageController.registerLanguageEx(
                        "agreen_tips",
                        "policy_all_update_link",
                        "%l1",
                        "device_user_agreement_title_update",
                        EMUI.privacyNoticeInfoController.eulaUrl,
                        "#007DFF"
            ...
            if (
                ...
                if (
                    ...
                    if (
                        ...
                        EMUI.LanguageController.registerLanguageEx(
                            ...
                            "understand_tip_1",
                            "%l",
                            "wireless_broadband_device_privacy_statement",
                            EMUI.privacyNoticeInfoController.privacyPolicyUrl,
                            "#007DFF"
                        );
                    } else {
                ...
                if (
                    ...
                    EMUI.LanguageController.registerLanguageEx(
                        ...
                        "agreen_tip_2",
                        "%l1",
                        "wireless_broadband_device_user_agreement",
                        EMUI.privacyNoticeInfoController.eulaUrl,
                        "#007DFF"
                    );
                    EMUI.LanguageController.registerLanguageEx(
            ...
            if (GLOBAL.modules && GLOBAL.modules.chrlog_enable === "1") {
                EMUI.userBehaviorController.initUserExperience(
                    "guide_userExperience_content"
                );
                $("#guide_privacy_content").css("max-height", "200px");
                EMUI.userBehaviorController.load(null, false);
                EMUI.userBehaviorController.initUserExperienceSwitch(
                    "user_experience_switch",
                    "check_off",
                    "check_on"
                );
                EMUI.LanguageController.registerLanguageEx(
                    "user_experience_label",
                    "IDS_user_experience_label",
                    "%l",
                    "menu.userExperience",
                    EMUI.privacyNoticeInfoController.userExperienceUrl,
                    "#007DFF"
                );
            }
            if (
        ...
        privacySecondPage: function () {
            $("#eulaTip").secureHtml(
                EMUI.privacyNoticeInfoController.simpleEula
            );
            $("#privacyTip").hide();
            $("#guide_userExperience_content").hide();
    ...
    EMUI.privacyNoticeController = EMUI.ObjController.extend({
        ...
        agreePrivacyNotice: function (disAgreen) {
            ...
            EMUI.privacyNoticeController.postData(
                requestPrivacy,
                function (result) {
                    if (
                        disAgreen === "0" &&
                        (EMUI.privacyNoticeInfoController.isFirstUpdate ||
                            EMUI.privacyNoticeInfoController
                                .isSecondaryAccountRefreshUpdate)
                    ) {
                    ...
                    if (result["errcode"] !== 0) {
                        if (
                            window.location.href.indexOf("fileshare") !== -1 ||
                            window.location.href.indexOf("fileUpload") !== -1
                        ) {
                        } else {
                        ...
                        EMUI.privacyNoticeInfoController.objName =
                            "app/privacypolicy?lang=" +
                            LANGUAGE_DATA.current_language;
                        EMUI.privacyNoticeInfoController.load(null, false);
                        var curPrivacyInfo =
                            EMUI.privacyNoticeInfoController.content;
                        if (EMUI.privacyNoticeInfoController.isFirstUpdate) {
                            if (
                                GLOBAL.isSecondaryAccount &&
                                (curPrivacyInfo.OperatorEULAUpdate === 1 ||
                                    curPrivacyInfo.OperatorPrivacyPolicyUpdate ===
                                        1)
                            ) {
                                window.location.reload();
                                return;
                            ...
                            if (
                                !GLOBAL.isSecondaryAccount &&
                                (curPrivacyInfo.IsPrivacyPolicyUpdate === 1 ||
                                    curPrivacyInfo.IsEULAUpdate === 1) &&
                                curPrivacyInfo.change_status !== 2
                            ) {
                                window.location.reload();
                                return;
                            }
                        ...
                        if (
                            !(
                                GLOBAL.deviceInfo &&
                                GLOBAL.deviceInfo.restore_default_status ===
                                    "1" &&
                                GLOBAL.modules &&
                        ...
                        if (
                            GLOBAL.modules &&
                            GLOBAL.modules.chrlog_enable === "1" &&
                            EMUI.privacyNoticeInfoController.isFirstUpdate
                        ) {
                            EMUI.userBehaviorController.submitGuideUserExpData();
                        }
                        if (window.location.href.indexOf("guidesmall") !== -1) {
                            $("#privacyStatement_updata_page").css(
                                "display",
                                "none"
                            );
                        } else if (
                            window.location.href.indexOf("guide.html") !== -1
                        ) {
                        ...
                        if (
                            window.location.href.indexOf("guide.html") !== -1 ||
                            window.location.href.indexOf("guidesmall") !== -1
                        ) {
                            $("#submit_fade").hide();
            ...
            if (window.location.href.indexOf("guidesmall") !== -1) {
                ...
                EMUI.privacyNoticeController.postData(
                    ...
                    function () {
                        ...
                        $("#autoUpg_checkBox").attr(
                            "src",
                            "../res/ic_check_on_normal.png"
                        );
                ...
                if (
                    ...
                    !EMUI.privacyNoticeInfoController
                        .isSecondaryAccountRefreshUpdate &&
                    EMUI.privacyNoticeInfoController.updateType !== "1"
                ) {
        ...
        showCancelPop: function () {
            ...
            utilStartConfirmDialog(
                publicLang["policy_update_cancel_content"],
                EMUI.privacyNoticeController.agreePrivacyNotice,
                function () {
                    EMUI.privacyNoticeController.agreePrivacyNotice("0");
                },
                publicLang["index.agreeBtn"],
    ...
    EMUI.CurrenLangObjController = EMUI.ObjController.extend({
        ...
        matchLanguageExist: function (currentLang, langListArr) {
            ...
            if ($.isArray(langListArr)) {
                ...
                for (var i = 0; i < arrayLen; i++) {
                    ...
                    if (
                        ...
                        LANGUAGE_DATA.current_language = langListArr[i].replace(
                            /-/,
                            "_"
                        );
                    }
            ...
            if (GLOBAL.modules.safeinfo_enable === "1") {
                if (
                    typeof LANGUAGE_DATA.safe_question_url_cpe !==
                        "undefined" &&
                    productInfo === "cpe"
                ) {
                    $("#safeinfo").attr(
                        "href",
                        LANGUAGE_DATA.safe_question_url_cpe[
                            LANGUAGE_DATA.current_language
                        ]
                    );
                } else if (
                    typeof LANGUAGE_DATA.safe_question_url_e5 !== "undefined" &&
                    (productInfo === "mobile-wifi" || productInfo === "touch")
                ) {
                    $("#safeinfo").attr(
                        "href",
                        LANGUAGE_DATA.safe_question_url_e5[
                            LANGUAGE_DATA.current_language
                        ]
                    );
                } else {
        ...
        adapterPrivacyurl: function () {
            ...
            if (
                ...
                devcap = EMUI.systemDeviceInfoexObjController.content.devcap
                    ? EMUI.systemDeviceInfoexObjController.content.devcap
                          .SoftwareCapability
                    : "";
            } else {
                devcap = EMUI.systemDeviceInfoObjController.content.devcap
                    ? EMUI.systemDeviceInfoObjController.content.devcap
                          .SoftwareCapability
                    : "";
            }
            var operatorName = "";
            if (
                ...
                if (LANGUAGE_DATA.current_language) {
                    EMUI.operatorInfoObjController.objName =
                        "app/operatorinfo?lang=" +
                        LANGUAGE_DATA.current_language;
                }
                EMUI.operatorInfoObjController.load(null, false);
                operatorName =
                    EMUI.operatorInfoObjController.content.OperatorName;
            }
            var region =
                "region=" +
                (GLOBAL.modules.china_region_enable !== undefined
                    ? GLOBAL.modules.china_region_enable
                    : "0");
            var privacyPower =
                "?" +
                region +
            ...
            var userExperienceUrlPower =
                ...
                (devcap["54"] !== undefined ? "54=" + devcap["54"] : "") +
                (devcap["108"] !== undefined ? "&108=" + devcap["108"] : "");
            EMUI.privacyNoticeInfoController.simpleEula = privacyXss(
                EMUI.privacyNoticeInfoController.content.SimpleEULA,
                "<br />"
            );
            EMUI.privacyNoticeInfoController.simplePrivacyPolicy = privacyXss(
                EMUI.privacyNoticeInfoController.content.SimplePrivacyPolicy,
                "<br />"
            );
            var privacyUrl = LANGUAGE_DATA.current_language
                ? "/policy/html/privacy/privacyPolicy-lang.html".replace(
                      "lang",
                      LANGUAGE_DATA.current_language
                  )
                : "/policy/html/privacy/privacyPolicy-en_us.html";
            var eulaUrl = LANGUAGE_DATA.current_language
                ? "/policy/html/protocol/protocol-lang.html".replace(
                      "lang",
                      LANGUAGE_DATA.current_language
                  )
                : "/policy/html/protocol/protocol-en_us.html";
            var userExperienceUrl = LANGUAGE_DATA.current_language
                ? "/policy/html/userexperience/UserExperienceProgram-lang.html".replace(
                      "lang",
                      LANGUAGE_DATA.current_language
                  )
                : "/policy/html/userexperience/UserExperienceProgram-en_us.html";
            EMUI.privacyNoticeInfoController.privacyPolicyUrl =
                privacyUrl + privacyPower + "&r=" + Math.random();
            EMUI.privacyNoticeInfoController.eulaUrl =
                eulaUrl + eulaPower + "&r=" + Math.random();
            EMUI.privacyNoticeInfoController.userExperienceUrl =
                userExperienceUrl +
                userExperienceUrlPower +
                "&r=" +
                Math.random();
            $("#footer_privacyPolicy").attr(
                "href",
                EMUI.privacyNoticeInfoController.privacyPolicyUrl
            );
            $("#footer_userAgreement").attr(
                "href",
                EMUI.privacyNoticeInfoController.eulaUrl
            );
            if (
                $("#guide_privacy_page").length > 0 &&
                $("#guide_privacy_page").css("display") !== "none"
            ) {
                if (
                    $("#guide_privacy_content #privacyTip").length > 0 &&
                    $("#guide_privacy_content #privacyTip").css("display") !==
                        "none"
                ) {
                    EMUI.privacyNoticeInfoController.privacyFirstPage();
                }
                if (
                    $("#guide_privacy_content #eulaTip").length > 0 &&
                    $("#guide_privacy_content #eulaTip").css("display") !==
                        "none"
                ) {
                    EMUI.privacyNoticeInfoController.privacySecondPage();
                }
        ...
        getsuccessProc: function (data) {
            ...
            var support_usermanual_language_list = $.makeArray(
                LANGUAGE_DATA.usermanual_language_list.support_language.language
            );
            var default_usermanual_language =
                LANGUAGE_DATA.usermanual_language_list.default_language;
            var helpAskqUrl =
                "../usermanual/" +
                default_usermanual_language +
                "/usermanual/faq_plugin/web_faq_topic_00001.html";
            for (
                var loop = 0;
                loop < support_usermanual_language_list.length;
                loop++
            ) {
                if (
                    data.response.CurrentLanguage ===
                    support_usermanual_language_list[loop]
                ) {
                    helpAskqUrl =
                        "../usermanual/" +
                        support_usermanual_language_list[loop] +
            ...
            getLang(lang, function () {
                if (
                    ...
                    GLOBAL.modules.dualwan_spread_name === "1"
                ) {
                    publicLang["menu.dualnetlink"] =
                        publicLang["menu.dualband"];
                    publicLang["bridge_mode_automatic_switch_prompt"] =
                        publicLang[
                            "bridge_mode_automatic_switch_prompt_dualband"
                        ];
                }
                ...
                if (
                    ...
                    $(".color_descroption_gray[lang-id=ailife_download_vendor]")
                        .parent()
                        .hide();
                }
                ...
                if (isShowCopyRight) {
                    if (lang === "zh_cn" && isShowCopyRight["zh_cn"]) {
                        $("#footer_copyright").html(
                            publicLang["footer.copyright.str"]
                                .replace("%d", xss(GLOBAL.config.copyright))
                                .replace("%s", xss(isShowCopyRight["zh_cn"]))
                        );
                    } else if (
                        (lang === "zh_hk" || lang === "zh_tw") &&
                        isShowCopyRight["zh_hk_tw"]
                    ) {
                        $("#footer_copyright").html(
                            publicLang["footer.copyright.str"]
                                .replace("%d", xss(GLOBAL.config.copyright))
                                .replace("%s", xss(isShowCopyRight["zh_hk_tw"]))
                        );
                    } else if (isShowCopyRight["common"]) {
                        $("#footer_copyright").html(
                            publicLang["footer.copyright.str"]
                                .replace("%d", xss(GLOBAL.config.copyright))
                                .replace("%s", xss(isShowCopyRight["common"]))
                        );
                    }
    ...
    EMUI.LanguageController = EMUI.ObjController.extend({
        ...
        registerLanguageEx: function (
            ...
            if (parastr.indexOf("%l") !== -1 && !paralink) {
                return;
            }
            var item = {};
        ...
        transLang: function () {
            ...
            for (i = 0; i < currRegLen; i++) {
                ...
                if (publicLang[item.resId].indexOf("%d") >= 0) {
                    $("#" + item["htmlId"]).html(
                        publicLang[item.resId].replace("%d", xss(item.para))
                    );
                } else if (publicLang[item.resId].indexOf("%s") >= 0) {
                    $("#" + item["htmlId"]).html(
                        publicLang[item.resId].replace(
                            "%s",
                            publicLang[xss(item.para)]
                                ? publicLang[xss(item.para)]
                                : xss(item.para)
                        )
                    );
            ...
            for (var i = 0; i < currRegLen; i++) {
                ...
                for (var j = 0; j < item.para.length; j++) {
                    ...
                    if (temppara.parastr.indexOf("%l") === -1) {
                        ...
                        var linkhtml =
                            '<a class="selectmenu" href="' +
                            xss(temppara.paralink) +
                            '" id="' +
                            xss(temppara.paravalue) +
                            '" rel="noopener noreferrer" target="_self" style="">' +
                            paraValueStr +
                            "</a>";
                        if (temppara.strColor) {
                            var linkhtml =
                                ...
                                '" id="' +
                                xss(temppara.paravalue) +
                                '" rel="noopener noreferrer" target="_blank" style="color:' +
                                temppara.strColor +
        ...
        adapterleftLabelStyle: function (para) {
            ...
            if (para) {
                var otherTag =
                    tag + " ." + para + " .control-label-win:visible";
    ...
    EMUI.ModelSwitchController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            if (data.type === "response") {
                ...
                if (
                    GLOBAL.modules.staticdns_enabled &&
                    GLOBAL.modules.staticdns_enabled !== "0"
                ) {
    ...
    EMUI.LinkAddressController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            if (data.type === "response") {
                if (
                    typeof data.response.custommenu_url !== "undefined" &&
                    data.response.custommenu_url !== ""
                ) {
    ...
    EMUI.ModelOnekeySwitchController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            if (data.type === "response") {
                GLOBAL.modules.onekeydiag_enabled =
                    data.response.onekeydiag_enabled;
            }
        },
    });
    ...
    EMUI.ModelDialupSwitchController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            if (data.type === "response") {
                GLOBAL.modules.tr069profile_enabled =
                    data.response.tr069_profile_display;
                GLOBAL.modules.iptvprofile_enabled =
                    data.response.iptv_profile_display;
                EMUI.ModelDialupSwitchController.voiceprofile_enabled =
                    data.response.voice_profile_display;
                GLOBAL.modules.wanmanagement_enabled =
                    data.response.multi_wan_enabled;
                GLOBAL.modules.wanprofile_enabled =
                    data.response.multi_wan_enabled;
            }
        },
    });
    EMUI.ModelFirewallSwitchController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            if (data.type === "response") {
                GLOBAL.modules.remotehttps_enabled =
                    data.response.firewallhttpsremoteswitch;
            }
        },
    });
    EMUI.policyRoutrController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            if (
                data.type === "response" &&
                data.response.lan_service_type &&
                data.response.lan_service_type !== "0"
            ) {
    ...
    EMUI.staticArpController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            if (
                data.type === "response" &&
                data.response.staticarp_display === "1"
            ) {
    ...
    EMUI.WifiFeatureSwitch = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            if (data.type === "response") {
                ...
                if (
                    typeof GLOBAL.wifiFeatureSwitch.wifispecialcharenable ===
                    "undefined"
                ) {
                    GLOBAL.wifiFeatureSwitch.wifispecialcharenable = "1";
                }
                if (
                    typeof GLOBAL.wifiFeatureSwitch.chinesessid_enable ===
                    "undefined"
                ) {
                    GLOBAL.wifiFeatureSwitch.chinesessid_enable = "0";
                }
                GLOBAL.modules.stawpsenabled =
                    GLOBAL.wifiFeatureSwitch.stawpsenabled;
                GLOBAL.modules.guestwifi_enable =
                    data.response.guestwifi_enable;
                GLOBAL.modules.dbho_special_enable =
                    GLOBAL.wifiFeatureSwitch.dbho_special_enable;
            }
        },
    });
    EMUI.isSuportWpa3 = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            if (data.type === "response") {
                ...
                if (
                    typeof isSuportWpa3[0].wifisupportsecmodelist ===
                        "undefined" ||
                    isSuportWpa3[0].wifisupportsecmodelist === ""
                ) {
                    GLOBAL.modules.wifi_wpa3_enable = "0";
                } else {
                    GLOBAL.modules.wifi_wpa3_enable = "1";
                    GLOBAL.modules.wifi_wpa3_list =
                        isSuportWpa3[0].wifisupportsecmodelist;
                }
            }
        },
    ...
    EMUI.RebootController = EMUI.ObjController.extend({
        ...
        showRebootInfo: function (isShowdynamic) {
            $("#submit_light").empty();
            var submithtml =
                '<div class="common_win_top"></div><div align="center" class="margin_bottom_box2 color_Darkgray color_background_white" style="font-size:16px;width:508px;padding-top:28px;min-height:180px;">';
            if (typeof isShowdynamic !== "undefined" && isShowdynamic) {
                submithtml +=
                    '<div style="padding-top:30px;"><img src="../res/submiting.gif" /></div>' +
                    '<div style="padding-top:25px;" lang-id="content.device.reboot">' +
                ...
                submithtml +=
                    ...
                    '<div lang-id="content.device.rebootnotice" style="font-size:16px;">' +
                    publicLang["content.device.rebootnotice"] +
                    "</div></div>" +
                    '<div style="padding-top:40px;padding-bottom: 8px; min-height:44px"><div class="btn_normal_short pull-left public_reboot_left" onclick="EMUI.RebootController.rebootCancel()">' +
                    publicLang["common_cancel"] +
                    "</div>" +
                    '<div class="btn_normal_short pull-left margin_left_12" onclick="EMUI.RebootController.rebootExe()">' +
        ...
        checkVoiceBusyState: function () {
            ...
            if (
                monitoringStatus &&
                monitoringStatus.voice_busy &&
                monitoringStatus.voice_busy === "1"
            ) {
        ...
        getsuccessProc: function (data) {
            if (data["type"] === "response") {
                ...
                if ("1" == responseData.EnableRedirection) {
                    ...
                    if (
                        responseData.Homepage.length > 4 &&
                        responseData.Homepage.toLowerCase().substring(0, 4) ==
                            "http"
                    ) {
    ...
    EMUI.webWebFeatureController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            ...
            GLOBAL.modules.captivePortalGuide =
                responceData["captive_portal_guide_enable"];
            this.controlGuideWifiSetEnable =
                responceData["guide_wifi_setting_enable"];
            this.controlAutoUpdataEnable =
                responceData["guide_auto_update_setting_enable"];
        },
    });
    EMUI.webAndWifiRuleController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            if (data.type === "response") {
                ...
                if (
                    parseInt(this.webMinLength) < 8 ||
                    parseInt(this.webMinLength) > 32
                ) {
                    this.webMinLength = 8;
                }
                if (
                    parseInt(this.wifiMinLength) < 8 ||
                    parseInt(this.wifiMinLength) > 63
                ) {
        ...
        showWifiErrorMsg: function (divid, errorlangid) {
            var diverrorlangid = (divid + errorlangid).replace(".", "");
            var wifiPwdMsg = publicLang[errorlangid].replace(
                "%d",
                this.wifiMinLength
            );
            EMUI.LanguageController.registerLanguage(
    ...
    EMUI.userBehaviorController = EMUI.ObjController.extend({
        ...
        initUserExperienceSwitch: function (
            switchID,
            disabledClass,
            enableClass
        ) {
        ...
        submitGuideUserExpData: function () {
            EMUI.userBehaviorController.UserExperienceData(
                "user_experience_switch"
            );
            EMUI.userBehaviorController.postData(
                EMUI.userBehaviorController.submitData,
                null,
                false
            );
        },
    ...
    EMUI.ping = (function () {
        ...
        function startPing(ipAddress, redirecturl) {
            ...
            pingIMG.onload = pingIMG.onerror = function () {
                ...
                if (redirecturl) {
                    gotoPageWithoutHistory(
                        "http://" + pingipAddress + "/html/" + redirecturl
                    );
                } else {
                    gotoPageWithoutHistory(
                        "http://" +
    ...
    EMUI.popWinController = EMUI.Object.extend({
        ...
        showPop: function () {
            ...
            for (i = 0; i < this.content.length; i++) {
                var contentItem = this.content[i];
                if (
                    contentItem["isClose"] === false &&
                    contentItem["moduleName"].toLowerCase().indexOf(curHash) < 0
                ) {
            ...
            if (contentLen === 0) {
                ...
                $("#pop_win_content_area").empty();
                $("#emui_pop_win_title").empty();
                $("#emui_pop_win_title").attr(
                    "lang-id",
                    showContent[0]["scribInfoId"]
                );
                ...
                for (i = 0; i < contentLen; i++) {
                    ...
                    var htmlItem =
                        '<div id="content_message_pop_' +
                        i +
                        '" onclick="EMUI.popWinController.toggleMessage(' +
                        i +
                        ');" class="clearboth list_shared  pointer ' +
                        classTop +
                        '">' +
                        '<div class="pull-left public_item_left" lang-id="' +
                        showContent[i]["scribInfoId"] +
                        '">' +
                        publicLang[showContent[i]["scribInfoId"]] +
                        "</div>" +
                        '<div id="arrow_content_message_pop_' +
                        i +
                        '" class="pull-right menu_arrow_close"></div></div><div class="clearboth"></div>' +
                        '<div class="border_top hide" id="body_content_message_pop_' +
                        i +
                        '" style="margin-top:20px;"><div style="margin-top:20px;">' +
    ...
    EMUI.indexAutoUpgradeController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            ...
            if (data["response"]["auto_update"] === "1") {
                ...
                if (
                    $("#index_autoUpg").attr("class").indexOf("tagName_tr") > -1
                ) {
                ...
                if (device_differentiation && device_differentiation == "0") {
                    ...
                    $(document).on(
                        "click",
                        "#index_autoUpg_check",
                        EMUI.indexAutoUpgradeController.autoUpgSwitch
                    );
                }
            }
        },
        autoUpgSwitch: function () {
            ...
            var className = $("#index_autoUpg_check").attr("class");
            if (
                className.substring(className.length - 2, className.length) ===
                "on"
            ) {
                className = className.replace("on", "off");
                EMUI.indexAutoUpgradeController.isOpenAutoUpg = 0;
    ...
    EMUI.devmanageCapacityObjController = EMUI.ObjController.extend({
        ...
        getsuccessProc: function (data) {
            if (
                data &&
                data.SoftwareCapability &&
                data.SoftwareCapability[81] === 1
            ) {
...
function getLangList() {
    getConfigData(
        "global/languagelist.xml",
        function ($xml) {
            var langListRet = xml2object($xml);
            if (langListRet.type === "config") {
                LANGUAGE_DATA.supportted_languages =
                    langListRet.config.languages.language;
                LANGUAGE_DATA.usermanual_language_list =
                    langListRet.config.usermanual_language;
                if (
                    GLOBAL.modules.safeinfo_enable === "1" &&
                    langListRet.config.safe_question_url
                ) {
                    LANGUAGE_DATA.safe_question_url_cpe =
                        langListRet.config.safe_question_url.cpe;
                    LANGUAGE_DATA.safe_question_url_e5 =
                        langListRet.config.safe_question_url.e5;
                    security_info_cpe =
                        langListRet.config.safe_question_url.cpe;
            ...
            if (globalConfig.type === "config") {
                ...
                if (globalConfig["config"]["titles_langAdapter"]) {
                    if (globalConfig["config"]["titles_langAdapter"][lang]) {
                        gDeviceTitle =
                            globalConfig["config"]["titles_langAdapter"][lang];
                    } else {
                        gDeviceTitle =
                            globalConfig["config"]["titles_langAdapter"][
                                "en-us"
                            ];
                    }
                } else {
                ...
                if (globalConfig["config"]["voice_busy_notice"]) {
                    GLOBAL.voiceBusyNotice =
                        globalConfig["config"]["voice_busy_notice"];
                } else {
                ...
                GLOBAL.homedeviceinfo =
                    globalConfig["config"]["homedeviceinfo"];
                document.title = gDeviceTitle;
                if (device_differentiation == "0") {
                    if ($(".guide_index_title").length > 0) {
                        $(".guide_index_title").html(
                            publicLang["guide.welcome"].replace(
                                "%s",
                                xss(gDeviceTitle)
                            )
                        );
...
function initAppQrcode(globalConfig) {
    if (window.location.href.indexOf("guidesmall.html") != -1) {
        if (
            globalConfig["config"]["appmanagements"]["enabled"] &&
            globalConfig["config"]["appmanagements"]["enabled"] === "1"
        ) {
            $("#app_install").attr(
                "href",
                globalConfig["config"]["appmanagements"]["hilink"]["domestic"]
            );
        }
        if (
            globalConfig["config"]["smarthomeapp"]["enabled"] &&
            globalConfig["config"]["smarthomeapp"]["enabled"] === "1"
        ) {
            $("#app_install").attr(
                "href",
                globalConfig["config"]["smarthomeapp"]["smartapp"]
            );
        }
    } else {
        if (
            ...
            $(
                "#qrcodeaddr, #commends_hilinkapp_download, .hilinkapp_download_href"
            ).attr(
                "href",
                globalConfig["config"]["appmanagements"]["hilink"]["domestic"]
            );
            $("#qrcodeaddr, .hilinkapp_download_href")
                .find("div")
                .removeClass("smart_qrcode")
                .addClass("hlink_qrcode");
            smartLife();
        }
        if (
            globalConfig["config"]["smarthomeapp"]["enabled"] &&
            ...
            $(
                "#qrcodeaddr, #commends_hilinkapp_download, .hilinkapp_download_href"
            ).attr("href", globalConfig["config"]["smarthomeapp"]["smartapp"]);
            $("#qrcodeaddr, .hilinkapp_download_href")
                .find("div")
                .removeClass("hlink_qrcode")
                .addClass("smart_qrcode");
            smartLife();
        }
        EMUI.webWebFeatureController.load(null, false);
        if (
            ...
            $(".hilinkapp_download_box .hilinkapp_download_href").removeAttr(
                "href"
            );
...
function showSelectedValue(selectid, selectval) {
    ...
    for (var loop = 0; loop < selectLength; loop++) {
        ...
        var selectvalue =
            selectitem.attr("option") || selectitem.find("span").attr("option");
        var description = selectitem.text() || selectitem.find("span").text();
        var selectlangid =
            selectitem.attr("lang-id") ||
            selectitem.find("span").attr("lang-id") ||
            "";
        if (selectval === selectvalue) {
            if (
                selectparent[0].localName !== "input" &&
                selectparent[0].localName !== "textarea"
            ) {
...
function utilGetTimeString(timeVal) {
    ...
    if (tmpVal > 0) {
        ...
        timeString +=
            '<span lang-id="' + langId + '">' + publicLang[langId] + "</span>";
    }
    timeInt %= 60 * 60 * 24;
...
function getCurrentTime(time) {
    ...
    if (finalTime !== "") {
        finalTime +=
            '<span lang-id="public.day">' +
            publicLang["public.day"] +
            "</span> ";
    }
...
function globalIntervalProc() {
    ...
    documentInterval = setInterval(function () {
        ...
        if (
            typeof GLOBAL.modules !== "undefined" &&
            typeof GLOBAL.modules.cbs_enabled !== "undefined" &&
            GLOBAL.modules.cbs_enabled === "1"
        ) {
...
function getMyAccountData() {
    if (GLOBAL.modules.myaccount === "1") {
        ...
        var myAccountHref = "";
        if (
            typeof GLOBAL.config.plmn_url !== "undefined" &&
            GLOBAL.config.plmn_url !== ""
        ) {
            myAccountHref =
                GLOBAL.config.plmn_url +
                "IMEI=" +
                myAccountStatus.encryp_imei +
                "&IMSI=" +
                myAccountStatus.encryp_imsi;
            $("#my_account").show();
...
$(document).ready(function () {
    ...
    if (
        GLOBAL.config.headaddbackground &&
    ...
    if (winHref.indexOf("/html/content.html") >= 0) {
        getConfigData(
            ...
            function ($xml) {
                var globalConfig = xml2object($xml);
                if (globalConfig.type === "config") {
                    GLOBAL.forbid_2g_upg_enable =
                        globalConfig["config"]["forbid_2g_upg_enable"];
                }
            },
            {
                sync: true,
        ...
        if (
            GLOBAL.modules["voip_enabled"] === "1" ||
            GLOBAL.modules["volte_enabled"] === "1"
        ) {
        ...
        if (
            EMUI.LoginStateController.content &&
            EMUI.LoginStateController.content.response &&
            EMUI.LoginStateController.content.response.State === "0"
        ) {
            ...
            function ($xml) {
                ...
                if (
                    EMUI.ModelDialupSwitchController.voiceprofile_enabled ===
                        "1" ||
                    (config !== "undefined" &&
                        (config.voip_data_switch_display_enable === "1" ||
                            config.voip_display_connection_status_enable ===
                                "1"))
                ) {
    ...
    if (
        typeof GLOBAL.modules !== "undefined" &&
        typeof GLOBAL.modules.help_enabled !== "undefined" &&
        GLOBAL.modules.help_enabled === "1"
    ) {
        $("#help_href").show();
    }
    ...
    $("body").click(function (e) {
        ...
        if (target && target.attr("class")) {
            ...
            if (
                classlist.indexOf("select_") < 0 &&
                classlist.indexOf("_dragger") < 0
            ) {
                $(".select_list").hide();
            }
        } else {
        ...
        if (
            ...
            targetID.indexOf("language_") < 0
        ) {
            $("#language_select_list").hide();
        }
        if (
            typeof targetID === "undefined" ||
            targetID.indexOf("statistics_select_") < 0
        ) {
            $("[class^=statistics_select_]").hide();
        }
        if (
            typeof targetID === "undefined" ||
            targetID.indexOf("device_status_") < 0
        ) {
            $("#device_status_list").hide();
        }
        if (
            typeof targetID === "undefined" ||
            targetID.indexOf("esimmanagement_status_") < 0
        ) {
            $("#esimmanagement_status_list").hide();
        }
        if (
            typeof targetID === "undefined" ||
            targetID.indexOf("dual_device_status_") < 0
        ) {
        ...
        if (tagetFLag) {
            if (tagetFLag.indexOf("_list_item_") > 0) {
                ...
                var parentselectid = tagetFLag.substr(
                    0,
                    tagetFLag.indexOf("_list_item_")
                );
                var selecthtml = $("#" + tagetFLag).text();
                $("#" + parentselectid + "_list > div").removeClass(
                    "select_top_selected select_bottom_selected select_medium_selected"
                );
                ...
                if (
                    $("#" + parentselectid)[0].localName !== "input" &&
                    $("#" + parentselectid)[0].localName !== "textarea"
                ) {
    ...
    if (
        LANGUAGE_DATA.current_language === "zh_cn" ||
        LANGUAGE_DATA.current_language === "zh_tw"
    ) {
...
VALIDATION.isValidDomainName = function (value) {
    var flag = "";
    if (
        !VALIDATION.isValidAscIICharset(value) ||
        !VALIDATION.isValidDomainNameChar(value)
    ) {
        flag = false;
    } else if (
...
VALIDATION.isSameSubnetAddrs = function (ip1, ip2, mask) {
    ...
    for (var i = 0; i < 4; i++) {
        if (
            (Number(addrParts1[i]) & Number(maskParts[i])) !==
...
VALIDATION.isValidNetworkSegment = function isValidNetworkSegment(
    netSeg,
    mask
) {
    ...
    for (var i = 0; i < 4; i++) {
        if (
            Number(addrParts[i]) !==
            (Number(addrParts[i]) & Number(maskParts[i]))
        ) {
            return false;
        }
    }
...
VALIDATION.isSameNetSegment = function isSameNetSegment(
    ipmax,
    ipmin,
    maskmax,
    maskmin
    ...
    for (var i = 0; i < 4; i++) {
        ...
        if (
            (Number(addrPartsmax[i]) & maskParts) !==
            (Number(addrPartsmin[i]) & maskParts)
        ) {
            return false;
        }
    }
...
VALIDATION.isVaildPortForIPFilter = function isVaildPortForIPFilter(
    port,
    showTarget
) {
    var portParts = $.trim(port);
    ...
    if (portParts.indexOf("-") === -1) {
        ...
        if (portParts.length === 2) {
            ...
            if (parseInt(portParts[0], 10) > parseInt(portParts[1], 10)) {
                showErrorMsg(
                    showTarget,
                    "firewall_hint_start_greater_end_port"
                );
                return false;
            }
        } else {
...
VALIDATION.checkInputPPPoeChar = function checkInputChar(str) {
    ...
    for (i = 0; i < str.length; i++) {
        ...
        if (
            numCharI > MACRO_SUPPORT_CHAR_MAX ||
            numCharI < MACRO_SUPPORT_CHAR_MIN
        ) {
            return false;
        }
    }
...
VALIDATION.checkInputChar = function checkInputChar(str) {
    ...
    for (i = 0; i < str.length; i++) {
        ...
        if (
            ...
            MACRO_NOT_SUPPORT_CHAR_COMMA === numCharI ||
            MACRO_NOT_SUPPORT_CHAR_QUOTATION_MARK === numCharI ||
        ...
        if (addrParts[1] !== "") {
            ...
            for (var i = 0; i < endAddress.length; i++) {
                ...
                for (m = 0; m < strLength; m++) {
                    ...
                    if (
                        !(
                            (asciiStr >= 48 && asciiStr <= 57) ||
                            (asciiStr >= 97 && asciiStr <= 102)
                        )
                    ) {
                        return "";
                    }
...
VALIDATION.isIPv6 = function isIPv6(strIP) {
    ...
    if (lastParts.indexOf("/") !== -1) {
        ...
        if (ipv6LastPart[0] !== "") {
            ...
            if (
                parseInt(ipv6LastPart[1], 10) > 128 ||
                parseInt(ipv6LastPart[1], 10) < 0
            ) {
                return false;
            }
            strIP = "";
    ...
    return (
        strIP.match(/:/g).length <= 7 &&
        (/::/.test(strIP)
            ? /^([\da-f]{1,4}(:|::)){1,6}[\da-f]{1,4}$/i.test(strIP)
            : /^([\da-f]{1,4}:){7}[\da-f]{1,4}$/i.test(strIP))
    );
};
VALIDATION.obverseMask = function (ip, mask) {
    ...
    obverseMaskAndIp = obverseMaskAndIp.substring(
        0,
        obverseMaskAndIp.length - 1
    );
...
function showTipInBridgeMode(disableAreaId) {
    ...
    $(".bridgemode_tip_info").length !== 0 &&
        $(".bridgemode_tip_info").remove();
    if (
        BridgeModeSwitch.content &&
        BridgeModeSwitch.content.response &&
        BridgeModeSwitch.content.response.bridgemode === "1"
    ) {
        GLOBAL.isBridgeModeOpen = true;
        if (
            ($.inArray(currentHash, voipPageArr) > -1 &&
                BridgeModeSwitch.content.response.voiceshareinternet === "0") ||
            ($.inArray(currentHash, tr069PageArr) > -1 &&
                BridgeModeSwitch.content.response.cwmpshareinternet === "0") ||
            ($.inArray(currentHash, videoPageArr) > -1 &&
                BridgeModeSwitch.content.response.iptvshareinternet === "0")
        ) {
            $("#" + disableAreaId).show();
            return false;
        }
        var bridgeModeTip =
            '<div class="clearboth text-left bridgemode_tip_info" style="margin-top:40px;margin-bottom:-10px">' +
...
function showTipInClat(pageName) {
    if (GLOBAL.modules.lan_ipmodeswitch_enabled === "1") {
        ...
        var clatTip =
            "" +
            '<div id = "clat_common_tip" class="bridgemode_tip_info page_scenes_info_text" style="display:table;margin-top:40px;">' +
            "</div>";
        $(".bridgemode_tip_info").length !== 0 &&
            $(".bridgemode_tip_info").remove();
        if (GLOBAL.ipmode === "1") {
            ...
            if (
                currentHash === "firewallswitch" &&
                GLOBAL.config.menu_v2.advanceset.security.lanipfilter ===
                    "lanipfilter"
            ) {
                if (GLOBAL.switchmode && GLOBAL.switchmode === "2") {
                    $("#" + pageName).show();
                } else {
                    $("#" + pageName).show();
                    $("#" + pageName).secureBefore(clatTip, null);
                    clat_notice_lang_id =
                        "iptype_clat_on_related_firwallswitch_page_message";
                }
            } else if (
                currentHash === "firewallswitch" &&
                GLOBAL.config.menu_v2.advanceset.security.lanipfilter ===
        ...
        if (GLOBAL.ipmode === "2") {
            if (
                currentHash === "vpnsettings" ||
                currentHash === "staticdns" ||
                currentHash === "acl"
            ) {
                $("#" + pageName).show();
                $("#" + pageName).secureBefore(clatTip, null);
                EMUI.LanguageController.registerLanguageEx(
                    "clat_common_tip",
                    "iptype_double_stack_ipv4_notice_message",
                    "%l",
                    "menu.iptype",
                    "#iptype"
                );
...
function isBridgeModeOpen() {
    ...
    if (
        BridgeModeOpen.content &&
        BridgeModeOpen.content.response &&
        BridgeModeOpen.content.response.bridgemode === "1"
    ) {
        GLOBAL.isBridgeModeOpen = true;
        if (
            BridgeModeOpen.content.response.bridgemodegatway &&
            BridgeModeOpen.content.response.bridgemodegatway !== ""
        ) {
            GLOBAL.bridgeSupportGatwayPage = [];
            if (
                BridgeModeOpen.content.response.bridgemodegatway.indexOf(";") >
                -1
            ) {
                GLOBAL.bridgeSupportGatwayPage =
                    BridgeModeOpen.content.response.bridgemodegatway.split(";");
            } else {
                GLOBAL.bridgeSupportGatwayPage.push(
                    BridgeModeOpen.content.response.bridgemodegatway
                );
            }
        }
...
function showTipInFirewall(disableId, switchname, errmsg) {
    ...
    $(".bridgemode_tip_info").length !== 0 &&
        $(".bridgemode_tip_info").remove();
    if (
        FirewallSwitch.content.response &&
        ...
        EMUI.LanguageController.registerLanguageEx(
            "firewall_common_tip",
            "IDS_security_message_firewall_disabled",
            "%l",
            "menu.firewallswitch",
            "#firewallswitch"
        );
...
function isHexString(str) {
    ...
    for (var i = 0; i < str.length; i++) {
        ...
        if (
            !(
                (c >= constNum.CHARCODE_0 && c <= constNum.CHARCODE_9) ||
...
function secondaryAccountFlag() {
    if (EMUI.BasicInfoObjController.content.response.classify === "hilink") {
        if (
            EMUI.loginallowedObjController.content.response.hilink_login === "1"
        ) {
    ...
    if (
        EMUI.LoginStateController.content.response &&
        EMUI.LoginStateController.content.response.accounts_number &&
        EMUI.LoginStateController.content.response.accounts_number === "2" &&
        EMUI.LoginStateController.content.response.userlevel &&
...
function addSelectItem(id, value) {
    ...
    var itemhtml =
        '<div id={{itemid}} option="{{option}}" class="select_medium" lang-id="{{langid}}" ><div style="direction: ltr;display: inline-block;text-indent: 0px;">{{text}}</div></div>';
...
function addSelectedBorder(obj) {
    ...
    for (var i = 0; i < curClassArray.length; i++) {
        if (
            curClassArray[i].indexOf("input") !== -1 &&
            curClassArray[i].indexOf("_selected") === -1
        ) {
...
function hideSelectedBorder(obj) {
    ...
    if (curClass) {
        curElement
            .parent()
            .removeClass(curClass)
            .addClass(curClass.substr(0, curClass.indexOf("_selected")));
    }
}
        ...
        $("input[placeholder]").each(function () {
            ...
            if (typeof pwdVal !== "undefined") {
                pwdField.after(
                    '<input id="' +
                        pwdId +
                        '1" style="color:#a0a0a0" lang-id-set="value" type="text" autocomplete="off" />'
                );
...
language_cultures["cs_cz"] =
    language_cultures["fi_fi"] =
    language_cultures["sl_sl"] =
    language_cultures["sr_cs"] =
        {
            numberFormat: ",",
...
language_cultures["de_de"] =
    language_cultures["mk_mk"] =
    language_cultures["no_no"] =
    language_cultures["ro_ro"] =
    language_cultures["tr_tr"] =
        {
            numberFormat: ",",
            timeFormat: "dd.MM.yyyy HH:mm:ss",
        };
...
language_cultures["fr_fr"] =
    language_cultures["it_it"] =
    language_cultures["pt_br"] =
        {
            numberFormat: ",",
            timeFormat: "dd/MM/yyyy HH:mm:ss",
        };
    ...
    for (;;) {
        ...
        switch (current) {
            ...
            case "dd":
                ...
                ret.push(padZeros(days, clength));
                break;
            case "M":
            case "MM":
                var months = value.getMonth() + 1;
                ret.push(padZeros(months, clength));
                break;
            case "yyyy":
                var years = value.getFullYear();
                ret.push(padZeros(years, clength));
                break;
            case "h":
            case "hh":
                var hours = value.getHours() % 12;
                if (hours === 0) hours = 12;
                ret.push(padZeros(hours, clength));
                break;
            case "H":
            case "HH":
...
function showDevHostOrActualName(devsHostArr) {
    for (var i = 0; i < devsHostArr.length; i++) {
        if (GLOBAL.modules.support_devcapacity) {
            if (
                (devsHostArr[i].DevBrands.toLowerCase() === "huawei" ||
                ...
                if (devsHostArr[i].ActualName === devsHostArr[i].HostName) {
                    ...
                    if (nameStr !== devsHostArr[i].MACAddress) {
                        ...
                        if (t > 0) {
                            if (
                                devsHostArr[i].HostName.charCodeAt(t + 1) <
                                    65 ||
                                devsHostArr[i].HostName.charCodeAt(t + 1) > 90
                            ) {
                                devsHostArr[i].HostName = devsHostArr[
                                    i
                                ].HostName.slice(0, t);
                            }
                        }
    ...
    $.each(hotlinkCell, function (index, value) {
        switch (value.type) {
            case "1":
                ...
                hotlinkDom +=
                    '<li><a class="txt signal_txt" href="javascript: void(0)" lang-id-set="title" lang-id="' +
                    hotlinkLang +
                    '"><span lang-id="' +
                    hotlinkLang +
                    '">' +
            ...
            case "3":
                ...
                if (hotlinkContainer.indexOf("foot") > -1) {
                    ...
                    hotlinkDom +=
                        '<li><a class="pic" id="' +
                        hotlinkPicId +
                        '"href="javascript: void(0)"></a><a class="txt" href="javascript: void(0)" lang-id-set="title" lang-id="' +
                        hotlinkLang +
                        '"><span lang-id="' +
                        hotlinkLang +
                        '">' +
            ...
            switch (value.click_area) {
                case "1":
                    $("#" + hotlinkContainer + " li")
                        .eq(index)
                        .find(".txt")
                        .attr({ href: value.link, target: "_blank" });
                    $("#" + hotlinkContainer + " li")
                        .eq(index)
                        .find(".txt")
                        .addClass("cursor");
